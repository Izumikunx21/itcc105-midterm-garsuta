Problem Statement: Why is the old menu-based portal inefficient?

-The current student portal uses a menu-based system that is often confusing and inefficient. Students have to go through multiple menus just to find simple information like schedules, grades, or announcements, which takes time and can be frustrating, especially for new users. Additionally, the system is not personalized, so all students see the same interface.

Industry 5.0 Justification: How will your AI empower humans rather than replace
them?

-The ADSSU AI Copilot follows the principles of Industry 5.0, where technology works alongside humans rather than replacing them. The AI serves as a smart assistant that helps students by providing instant answers, recommendations, and guidance, making interactions faster and easier. At the same time, staff still manage the system, while the AI handles repetitive tasks like answering common questions and assisting with navigation.

Agile Requirements: Write three (3) User Stories using the format:
'As a [User], I want [AI Feature], so that [Benefit].'

-As a Student, I want an AI chatbot interface, so that I can quickly access information without navigating menus.
-As a Student, I want the AI to understand English and Bisaya, so that I can interact comfortably using my preferred language.
-As a Student, I want personalized responses from the AI, so that I receive relevant information based on my profile.
